...
| ~~Array / list column type~~ **built; not generated** | Drizzle, SQLAlchemy, Ecto | `ValueType::Array` exists, `array_column(name, element)` declares one, and the kernel stores, reads, compares and sorts arrays — element-wise with a shorter list first, from a terminated encoding rather than a length prefix, which is the one decision a serialisation format would get backwards. Decided in [`arrays.md`](arrays.md) and built in `crates/slate-kernel/tests/arrays.rs`. The wire carries one, `slate-serverd` declares one as `type = "array", element = "str"`, and Go, Python and TypeScript each send and receive one — proved against a real node in each. **What is left is the generator**: `scripts/codegen.py` refuses an array column deliberately, saying what it would need (the declaration has to carry the element type, and the decoded form is element-typed in three languages). The predicate parser now takes `tags = ['a', 'b']`, with the column deciding the element type and with nesting, a `NULL` element and a column reference inside a list each refused for a reason the kernel already had. An array is still refused in a key and an index, because the question asked of one is *containment* — one index entry per element, the same new cardinality full-text search needs |
