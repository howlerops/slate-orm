...
| Window functions | SQLAlchemy, Drizzle, Diesel | aggregates are `Count, CountColumn, Min, Max, Sum, Avg, CountDistinct` — **and this is a new operator, not new variants.** `Grouper` holds `HashMap<encoded_key, (Vec<Value>, Accumulators)>`: it folds each row into its group's accumulators and **discards the row**. A window function emits one output row per *input* row, with a value computed over the partition — the opposite cardinality. So it cannot be added to the aggregate list; it needs an operator that preserves rows, which also changes what `max_groups` and `max_sort_rows` are bounding |
| CTEs / recursive queries | SQLAlchemy, Drizzle, Diesel | no plan node — **and the same tension as set operations above, unargued.** One statement compiles to one `QuerySpec` naming one table and one plan; a CTE is a second named query in the same statement. Whether that makes this Refused (like `UNION`) or Missing is a question nobody has answered in writing, and the row has been asserting the second for as long as it stood |
...
| Views | Drizzle, SQLAlchemy | none — **and there is a security hazard to design around before any code.** Grants and policies are keyed on `TableId` (`Grant { table: TableId }`, `Policy { table: TableId }`), and a query resolves its table by name through `Catalog::table_by_name`. Give a view its own `TableId` so that lookup finds it, and every grant and policy check keys on the *view's* id — a caller granted the view reads the base table's rows with the base table's policy never consulted. The safe shape is that a view expands to its underlying spec *before* planning, so the base table's id is what reaches the policy, and that has to be structural rather than a convention somebody remembers |
| ~~Array / list column type~~ **kernel built; not yet on the wire** | Drizzle, SQLAlchemy, Ecto | `ValueType::Array` exists, `array_column(name, element)` declares one, and the kernel stores, reads, compares and sorts arrays — element-wise with a shorter list first, from a terminated encoding rather than a length prefix, which is the one decision a serialisation format would get backwards. Decided in [`arrays.md`](arrays.md) and built in `crates/slate-kernel/tests/arrays.rs`. **What is left is everything outside the kernel**: the wire `Value` message, the three clients, the generated row types, `slate-serverd`'s TOML schema and a SQL literal all have no case, so no client can send or receive one. An array is still refused in a key and an index, because the question asked of one is *containment* — one index entry per element, the same new cardinality full-text search needs |
