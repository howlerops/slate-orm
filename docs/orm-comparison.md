...
| Array / list column type | Drizzle, SQLAlchemy, Ecto | `ValueType` has no `Array`. The only one of these rows that is ordinary work rather than blocked or already decided — so the decisions are written down first, in [`arrays.md`](arrays.md): the element type lives on the column as a decimal's scale does, ordering is lexicographic with shorter-is-less (a length prefix cannot give that), and the first version refuses an array in a key or an index because what a user wants there is *containment*, which is the same new index cardinality full-text search needs |
