...
- A computed value has no slot in a *joined* row, because the kernel packs a
  joined row by declared table width. Naming one across inputs is refused by
  the server with that reason. This client does not try to pre-empt the
  refusal: the server's message is better than anything written here, and a
...
UnaryOp = Literal["length", "lower", "upper"]
