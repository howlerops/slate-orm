...
import { bool, bytes, float, ge, int, isKind, nullValue, str, uint, uuid, } from "../src/index.js";
const servers = [];
async function serving(extra = "") {
...
test("insert then get", async () => {
    ...
    assert.equal(row[1].value, "note");
    assert.equal(row[2].value, 10n);
});
...
test("insert refuses a duplicate key", async () => {
    ...
    await assert.rejects(() => session.insert("docs", row), (error) => {
        assert.ok(isKind(error, "already-exists"), `kind was ${error.kind}`);
        assert.equal(error.retryable, false, "a taken key is not retryable");
        return true;
    });
});
...
test("upsert replaces where insert refuses", async () => {
    ...
    const row = await session.get("docs", [uint(1)]);
    assert.equal((row?.[1]).value, "second");
});
test("query filters and orders", async () => {
    ...
    assert.equal((rows[0]?.[2]).value, 50n);
    assert.equal((rows[2]?.[2]).value, 30n);
});
...
`);
    ...
    assert.ok(row);
    assert.equal(row[1].value, true);
    assert.deepEqual(Array.from(row[2].value), [0xde, 0xad]);
    assert.equal(row[3].value, "hello");
    assert.equal(row[4].value, -5n);
    expectUint(row[5], 5n, "the u64 column");
    assert.equal(row[6].value, 1.5);
    assert.deepEqual(Array.from(row[7].value), Array.from(id));
...
`);
    const session = server
        ...
        .session();
    await assert.rejects(() => session.explain({ table: "plain" }), (error) => {
        assert.ok(isKind(error, "permission-denied"), `a read grant must not carry EXPLAIN, got ${error.kind}`);
        return true;
    });
});
...
test("the session watermark advances on write", async () => {
    ...
    assert.ok(second !== undefined && second > first, "a second write must advance it further, not merely set it once");
});
test("an unknown table is not-found", async () => {
    const session = (await serving()).client().session();
    await assert.rejects(() => session.get("nosuchtable", [uint(1)]), (error) => {
        assert.ok(isKind(error, "not-found"), `kind was ${error.kind}`);
        return true;
    });
});
...
test("a null in a non-nullable column is refused", async () => {
    const session = (await serving()).client().session();
    await assert.rejects(() => session.insert("docs", [uint(1), nullValue, int(0)]), (error) => {
        assert.ok(isKind(error, "invalid-request"), `kind was ${error.kind}`);
        assert.match(error.message, /nullable/);
        return true;
    });
});
