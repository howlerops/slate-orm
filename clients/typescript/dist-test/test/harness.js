...
function repositoryRoot() {
    ...
    for (;;) {
        ...
        if (parent === dir)
            throw new Error(`no repository root above ${HERE}`);
        dir = parent;
    }
}
...
function binary() {
    if (!built) {
        const result = spawnSync("cargo", ["build", "-p", "slate-serverd", "--bin", "slate-serverd"], { cwd: ROOT, encoding: "utf8" });
        if (result.status !== 0) {
