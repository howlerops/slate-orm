...
import { agg, at, count, groupGt, groupKey, int, key0, maxOf, newJoin, str, uint, } from "../src/index.js";
...
function authorsBooks(type = "inner") {
    const b = newJoin();
...
test("a left join keeps the unmatched left side", async () => {
    ...
    assert.equal(rows.filter((row) => row[1] === undefined).length, 1, "exactly author 3 is unmatched");
});
...
test("a grouped aggregate", async () => {
    ...
    const groups = await session
        .aggregate({ table: "books" }, { groupBy: [key0(1)], aggregates: [count(), maxOf(key0(3))] })
        .collect();
    ...
    assert.equal(first.values[1].value, 2003n, "author 1's latest year");
});
...
test("every join algorithm agrees", async () => {
    const session = await library();
    const answers = [];
    for (const force of [undefined, "hash-build-left", "hash-build-right", "nested-loop"]) {
        const b = newJoin();
        const authors = b.add({ table: "authors" });
        b.add({
            table: "books",
