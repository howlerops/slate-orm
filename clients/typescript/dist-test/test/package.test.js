...
function packageRoot() {
    ...
    for (;;) {
        ...
        if (existsSync(manifest)) {
            ...
            if (parsed.name === "@slate-orm/client")
                return dir;
        }
        const parent = path.dirname(dir);
        if (parent === dir)
            throw new Error("no @slate-orm/client package root found");
        dir = parent;
    }
}
...
test("the package's exports point at files the build produces", () => {
    ...
    const referenced = new Set([manifest.main, manifest.types]);
    ...
    for (const target of referenced) {
        assert.ok(existsSync(path.join(ROOT, target)), `package.json references ${target}, which the build does not produce`);
    }
});
