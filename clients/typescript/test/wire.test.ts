...
import { applyGrouping, at, count, type Aggregate } from "../src/join.js";
...
test("COUNT(*) sends no column, even when one is set", () => {
  ...
  const handBuilt: Aggregate = { function: "count", column: at(0, 3) };
  const wire = applyGrouping({}, { aggregates: [handBuilt] });

  const aggregates = wire["aggregates"] as Record<string, unknown>[];
  assert.equal(aggregates.length, 1);
  assert.equal(aggregates[0]!["function"], "AGGREGATE_FUNCTION_COUNT");
  assert.ok(
    !("column" in aggregates[0]!),
    `COUNT(*) must send no column, got ${JSON.stringify(aggregates[0])}`,
  );
});
