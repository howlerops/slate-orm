...
import { Client, str, uint } from "@slate-orm/client";
...
async function timeSingles(session, base, rows) {
...
async function timeBatch(session, base, rows) {
    const inserts = [];
    for (let n = 0; n < rows; n += 1)
        inserts.push([uint(base + n), str("row")]);
    const start = process.hrtime.bigint();
    await session.batch({
...
function report(name, rows, singles, batches) {
    const per = (xs) => xs.map((x) => (x / rows) * 1e6).sort((a, b) => a - b);
    const median = (xs) => xs.length % 2 === 1 ? xs[(xs.length - 1) / 2] : (xs[xs.length / 2 - 1] + xs[xs.length / 2]) / 2;
    const s = per(singles);
    const b = per(batches);
