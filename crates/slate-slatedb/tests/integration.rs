//! The record layer running against a real SlateDB instance.
//!
//! The kernel's own suite runs against an in-memory backend that reproduces
//! SlateDB's semantics. These tests check that the reproduction is faithful on
//! the points the record layer actually depends on: atomic multi-key commits,
//! ordered range scans in both directions, and write-write conflict detection
...
async fn store() -> RecordStore<SlateStore> {
...
#[tokio::test]
async fn writes_and_reads_survive_a_round_trip() {
...
/// A commit either lands whole or not at all, across the row and every index
/// entry it touches.
#[tokio::test]
async fn a_failed_transaction_leaves_no_partial_index_state() {
    let store = store().await;
    let table = users();
    ...
    // Roll back a transaction that wrote a row and its index entries.
    let txn = store.begin().await.unwrap();
    ...
    // The rolled-back row's index entry must be gone too, or a later query by
    // email would resolve to a row that does not exist.
    let by_email = table.ordinal_of("email").unwrap();
    let hits = txn
        .query(
            &root(),
            &table,
            Expr::eq(by_email, Value::Str("gone@x.com".into())),
            ScanOrder::Ascending,
        )
        .await
        .unwrap()
    ...
    assert!(
        hits.is_empty(),
        "index entry outlived a rolled-back row: {hits:?}"
    );

    store.backend().close().await.unwrap();
}

/// The guarantee that made unique indexes cheap: two writers producing the same
/// index key are separated by SlateDB's own conflict detection.
#[tokio::test]
async fn concurrent_writers_cannot_both_take_a_unique_slot() {
    let store = store().await;
    let table = users();
...
/// Sequential duplicates get the friendly error rather than a bare retry.
#[tokio::test]
async fn a_sequential_duplicate_reports_the_index_it_hit() {
    let store = store().await;
    let table = users();
...
/// Range scans have to come back in key order, in both directions, or index
/// order means nothing.
#[tokio::test]
async fn scans_are_ordered_in_both_directions() {
    let store = store().await;
    let table = users();
...
/// Index maintenance across a real commit boundary: the stale entry must be
/// gone from storage, not merely shadowed.
#[tokio::test]
async fn an_update_retires_the_old_index_entry() {
    let store = store().await;
    let table = users();
...
/// Security holds over the real backend too: the tenant term becomes a key
/// prefix and the policy filters what is left.
#[tokio::test]
async fn tenant_scoping_and_policies_hold_over_slatedb() {
    let store = store().await;
    let table = users();
    ...
    assert_eq!(
        emails,
        vec!["adult@a.com"],
        "tenant B or the under-age row leaked"
    );
