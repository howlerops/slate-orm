...
//! same object store. The process-level state — memtable, block cache, the
//! catalog and security objects, every cursor — is gone; the object store is
//! what carries over. That is the same boundary a crashed and restarted
//! process crosses, minus the part where it fails mid-write, which
...
use common::{TENANT_A, TENANT_B, member, pk, root, user, users};
...
/// Open a store over `object_store`, as a fresh process would.
async fn open(object_store: Arc<dyn ObjectStore>) -> RecordStore<SlateStore> {
    let backend = SlateStore::open("/records", object_store)
        .await
        .expect("open slatedb");
...
/// The control for every test here: a store opened over a *different* object
/// store sees nothing.
///
/// Without this, all of the above could pass for the wrong reason — data
/// reaching the second store through some ambient process state rather than
/// through the object store, which would make "survives a restart" mean
/// nothing at all.
#[tokio::test]
async fn a_different_object_store_is_a_different_database() {
    let table = users();

    let written: Arc<dyn ObjectStore> = Arc::new(InMemory::new());
    let store = open(Arc::clone(&written)).await;
    let txn = store.begin().await.unwrap();
    txn.insert(
        &root(),
        &table,
    ...
    // Same path, same schema, same everything but the bytes underneath.
    let elsewhere = open(Arc::new(InMemory::new())).await;
    assert!(
        all_rows(&elsewhere, TENANT_A).await.is_empty(),
        "a row appeared in a store that was never written to, so these tests \
         are not measuring durability"
    );
    close(elsewhere).await;

    // And the original still has it, so the control did not disturb it.
    let reopened = open(written).await;
...
#[tokio::test]
async fn committed_rows_survive_a_restart() {
    let object_store: Arc<dyn ObjectStore> = Arc::new(InMemory::new());
    let table = users();

    let before = {
        let store = open(Arc::clone(&object_store)).await;
        let txn = store.begin().await.unwrap();
    ...
    let store = open(object_store).await;
...
/// the entries decode.
#[tokio::test]
async fn a_unique_index_still_constrains_after_a_restart() {
    let object_store: Arc<dyn ObjectStore> = Arc::new(InMemory::new());
    let table = users();
...
#[tokio::test]
async fn the_index_and_the_table_agree_after_a_restart() {
    let object_store: Arc<dyn ObjectStore> = Arc::new(InMemory::new());
    let table = users();
...
///
/// This is the crash case approached from the side that can be tested
/// deterministically: the writes happened, the commit did not, and the store
/// was reopened.
#[tokio::test]
...
/// a restart can lose.
#[tokio::test]
async fn a_delete_stays_deleted_across_a_restart() {
    let object_store: Arc<dyn ObjectStore> = Arc::new(InMemory::new());
    let table = users();
...
#[tokio::test]
async fn tenant_isolation_holds_after_a_restart() {
    let object_store: Arc<dyn ObjectStore> = Arc::new(InMemory::new());
    let table = users();
...
#[tokio::test]
async fn writes_after_a_restart_survive_the_next_one() {
    let object_store: Arc<dyn ObjectStore> = Arc::new(InMemory::new());
    let table = users();
...
#[tokio::test]
async fn a_covering_scan_agrees_with_the_table_after_a_restart() {
    let object_store: Arc<dyn ObjectStore> = Arc::new(InMemory::new());
    let table = users();
    let age = ordinal("age");
